...
describe('spectra-data test peak picking in ACS format', function () {
    ...
    it('format ACS new input format', function () {
        console.log(peakPicking2);
        var acs = Data.ACS2.formater.toACS(peakPicking2,{rangeForMultiplet:true, nucleus:spectrum.getNucleus(), observe:spectrum.observeFrequencyX()});
        acs.should.equal("<sup>1</sup>H NMR (400 MHz) δ 7.26-7.31 (m, 2 H), 7.15-7.22 (m, 3 H), 2.60 (q, 2 H, <i>J</i> = 7.6 Hz), 1.19 (t, 3 H, <i>J</i> = 7.6 Hz).");
    });
});
