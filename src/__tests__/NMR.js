...
describe('spectra-data examples ethylvinylether/1h.jdx', function () {
    ...
    it('Check peak-picking in the new format', function () {
        var peakPicking = spectrum.autoPeakPicking({'nH': 8, realTop: true, thresholdFactor: 1, clean: true, compile: true, idPrefix: '1H', format: 'new'});
        //console.log(peakPicking[0]);
        peakPicking[0].signal[0].peak.length.should.equal(4);
    });

    it('Check peak-picking in zone', function () {
        var peakPicking = spectrum.autoPeakPicking({'nH': 8, realTop: true, thresholdFactor: 1, clean: true, compile: true, idPrefix: '1H', format: 'new', from: 1, to: 2});
        //console.log(peakPicking[0]);
    ...
    it('updateIntegrals', function () {
        var nH = 8;
        var ranges = spectrum.autoPeakPicking({'nH': nH, realTop: true, thresholdFactor: 1, clean: true, compile: true, idPrefix: '1H', format: 'new'});
        ranges[0].to = 6.47;
