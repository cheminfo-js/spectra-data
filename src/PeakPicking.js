...
module.exports = function (spectrum, optionsEx) {
    ...
    var i, j, nHi, sum;
//var noiseLevel = Math.abs(spectrum.getNoiseLevel())*(options.thresholdFactor)   ; si pasa un noiseLevel porque volverlo a calcular
    var noiseLevel = Boolean(options.noiseLevel === undefined) ? Math.abs(spectrum.getNoiseLevel()) * (options.thresholdFactor) : Math.abs(options.noiseLevel) * (options.thresholdFactor);
    ...
    var signals = detectSignals(peakList, spectrum, options.nH, options.integralFn, options.absoluteIntegral);
 ...
 */
function detectSignals(peakList, spectrum, nH, integralType, absoluteIntegral) {
    ...
    if(absoluteIntegral ? false : true){
        for(var i=0;i<signals.length;i++){
            var integral = signals[i].integralData;
